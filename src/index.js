...
function getAdminIds() {
  const admins = data.admins().admins || [];
  return [...new Set([OWNER_ID, ...admins.map(Number)].filter(Boolean))];
}
async function notifyAdmins(ctx, text, extra = {}) {
  const fromId = uid(ctx);
  for (const adminId of getAdminIds()) {
    if (Number(adminId) === Number(fromId)) continue;
    try { await ctx.telegram.sendMessage(adminId, text, { parse_mode: 'HTML', disable_web_page_preview: true, ...extra }); } catch {}
  }
}
function findUserIdByRef(ref) {
  if (!ref) return null;
  const clean = String(ref).trim().replace('@', '').toLowerCase();
  if (/^\d+$/.test(clean)) return Number(clean);
  const users = data.users();
  const found = Object.values(users).find(u => String(u.username || '').toLowerCase() === clean);
  return found?.id ? Number(found.id) : null;
}
const ADMIN_PANEL_TEXT = `🛠 <b>MD STORE Admin Panel</b>
...
async function createDisabledOrder(ctx, paymentMethod) {
  ...
  const orderLines = order.items.map(x => `${x.title} × ${x.qty}`).join('\n');
  const safeOrderLines = orderLines.replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const targetInfo = order.orderType === 'prime'
    ? `Telegram: ${order.primeTarget || '-'}`
    : `PUBG ID: ${order.pubgId || '-'}`;
  await notifyAdmins(ctx, `🛒 <b>New order received</b>

Order: <code>${order.id}</code>
User: ${userLabelFromCtx(ctx)}
Service: <b>${order.orderType === 'prime' ? 'Prime Plus' : 'PUBG UC'}</b>
${targetInfo}

Items:
<code>${safeOrderLines}</code>

Total: <b>${rub(order.totalRub)}</b>
Payment: <b>${paymentMethod}</b>
Status: <code>${order.status}</code>

↩️ Reply: <code>/reply ${ctx.from.id} MESSAGE</code>`);
...
// Notify owner/admins when a new user enters and when users send messages.
bot.use(async (ctx, next) => {
  if (!ctx.from) return next();
  const usersBefore = data.users();
  const wasKnown = Boolean(usersBefore[ctx.from.id]);
  const user = ensureUser(ctx);

  if (!wasKnown && !isAdminId(ctx.from.id)) {
    await notifyAdmins(ctx, `🆕 <b>New user entered the bot</b>\n\n👤 ${userLabelFromCtx(ctx)}\n🌐 Lang: ${user.lang || 'ru'}\n🕒 ${nowIso()}`);
  }

  if (ctx.message && !isAdminId(ctx.from.id)) {
    let content = '';
    if (ctx.message.text) content = `💬 <code>${String(ctx.message.text).slice(0, 3500).replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code>`;
    else if (ctx.message.photo) content = '🖼 Photo';
    else if (ctx.message.document) content = `📎 Document: ${ctx.message.document.file_name || ''}`;
    else if (ctx.message.sticker) content = '🌟 Sticker';
    else content = `📩 Message type: ${Object.keys(ctx.message).join(', ')}`;
    await notifyAdmins(ctx, `📥 <b>User message</b>\n\n👤 ${userLabelFromCtx(ctx)}\n${content}\n\n↩️ Reply: <code>/reply ${ctx.from.id} MESSAGE</code>`);
  }
  return next();
});
...
bot.command('admin', (ctx) => { if (!adminOnly(ctx)) return; ctx.reply(ADMIN_PANEL_TEXT, { parse_mode: 'HTML' }); });
...
bot.command('addadmin', (ctx) => {
  if (uid(ctx) !== OWNER_ID) return ctx.reply('Owner only');
  const [ref] = cmdArgs(ctx);
  if (!ref) return ctx.reply('/addadmin USER_ID_OR_USERNAME');
  const n = findUserIdByRef(ref);
  if (!n) return ctx.reply('Admin not found. Ask this user to open the bot first, then use /addadmin USER_ID or /addadmin @username');
  const obj = data.admins();
  if (!obj.admins.map(Number).includes(Number(n))) obj.admins.push(Number(n));
  data.saveAdmins(obj);
  ctx.reply(`✅ Admin added: ${n}`);
});
bot.command('deladmin', (ctx) => {
  if (uid(ctx) !== OWNER_ID) return ctx.reply('Owner only');
  const [ref] = cmdArgs(ctx);
  if (!ref) return ctx.reply('/deladmin USER_ID_OR_USERNAME');
  const id = findUserIdByRef(ref) || Number(String(ref).replace('@',''));
  if (Number(id) === OWNER_ID) return ctx.reply('Cannot remove owner');
  const obj = data.admins();
  obj.admins = obj.admins.filter(x => Number(x) !== Number(id));
  data.saveAdmins(obj);
  ctx.reply(`✅ Admin removed: ${id}`);
});
