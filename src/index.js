...
function mainKeyboard(lang = 'ru') {
  // Telegram does not allow developers to force exact button colors in normal reply keyboards.
  // The buttons will follow Telegram client theme colors; emojis/custom text are used for styling.
  return Markup.keyboard([
    [Markup.button.webApp(`💝 ${tr(lang, 'openApp')}`, MINI_APP_URL)],
    [`💰 ${tr(lang, 'buyUc')}`],
    [`⭐ ${tr(lang, 'prime')}`, `ℹ️ ${tr(lang, 'info')}`],
    [`🛟 ${tr(lang, 'support')}`, `📣 ${tr(lang, 'channel')}`],
    [`🌐 ${tr(lang, 'lang')}`]
  ]).resize();
}
function cancelKeyboard(lang = 'ru') {
  return Markup.keyboard([[`❌ ${tr(lang, 'cancel')}`], [`🏠 ${tr(lang, 'home')}`]]).resize();
}
function homeButton(lang = 'ru') {
  return Markup.keyboard([[`🏠 ${tr(lang, 'home')}`]]).resize();
}
function inlineSiteButtons(lang = 'ru') {
  return Markup.inlineKeyboard([
    [Markup.button.webApp(`💝 ${tr(lang, 'openApp')}`, MINI_APP_URL)],
    [Markup.button.url(`📣 ${tr(lang, 'channel')}`, CHANNEL_URL), Markup.button.url(`🛟 ${tr(lang, 'support')}`, SUPPORT_URL)]
  ]);
}
...
async function sendWelcome(ctx) {
  ...
  const caption = `<b>${tr(lang, 'welcome', user.firstName)}</b>\n\n${CUSTOM.app} <b>${BOT_NAME}</b>\n${CUSTOM.uc} <b>Premium PUBG Mobile Store</b>`;
  if (fs.existsSync(WELCOME_IMAGE)) {
    await ctx.replyWithPhoto({ source: WELCOME_IMAGE }, { caption, parse_mode: 'HTML', ...inlineSiteButtons(lang) });
  } else {
    await ctx.reply(caption, { parse_mode: 'HTML', ...inlineSiteButtons(lang) });
  }
  await ctx.reply(`🏠 ${tr(lang, 'main')}`, mainKeyboard(lang));
}

function getSession(ctx) {
  const id = uid(ctx);
  if (!sessions.has(id)) sessions.set(id, { mode: null, cart: {}, pubgId: null, payment: null });
  return sessions.get(id);
}
function resetSession(ctx) { sessions.set(uid(ctx), { mode: null, cart: {}, pubgId: null, payment: null }); }
...
async function showUcMethod(ctx) {
  ...
  await ctx.reply(tr(lang, 'chooseUcWay'), Markup.keyboard([
    [`🆔 ${tr(lang, 'byId')}`],
    [`🔐 ${tr(lang, 'byCode')}`],
    [`❌ ${tr(lang, 'cancel')}`]
  ]).resize());
...
async function showUcCart(ctx) {
  ...
  const rows = products.filter(p => p.active !== false).map(p => {
    ...
    if (q > 0) return [`➕ ${p.title} (${q}) - ${rub(p.priceRub)}`, `➖ ${lang === 'ru' ? 'Удалить' : 'Remove'}`];
    return [`➕ ${p.title} - ${rub(p.priceRub)}`];
  });
  rows.push([`✍️ ${tr(lang, 'customAmount')}`]);
  rows.push([`✅ ${tr(lang, 'checkout')}`]);
  rows.push([`🧹 ${tr(lang, 'clearCart')}`, `🏠 ${tr(lang, 'home')}`]);
  const text = `🛒 <b>${tr(lang, 'cart')}:</b>\n\n• ${tr(lang, 'totalUc')}: <b>${cartUc(s)} UC</b>\n• ${tr(lang, 'total')}: <b>${rub(cartTotal(s))}</b>\n\n<b>${tr(lang, 'chooseProducts')}</b>`;
  await ctx.reply(text, { parse_mode: 'HTML', ...Markup.keyboard(rows).resize() });
}

async function showPrime(ctx) {
  const lang = langOf(ctx);
  const rows = (data.products().prime_plus || []).filter(p => p.active !== false).map(p => [`⭐ ${p.title} - ${rub(p.priceRub)}`]);
  rows.push([`🏠 ${tr(lang, 'home')}`]);
  const text = `${CUSTOM.prime} <b>${tr(lang, 'primeTitle')}</b>`;
  await ctx.reply(text, { parse_mode: 'HTML', ...Markup.keyboard(rows).resize() });
}
...
async function confirmOrder(ctx) {
  const lang = langOf(ctx);
  const s = getSession(ctx);
  if (!cartItems(s).length) return ctx.reply(tr(lang, 'empty') || 'Empty', mainKeyboard(lang));
  if (!s.pubgId) {
    s.mode = 'await_pubg_id';
    return ctx.reply(`✅<b>${tr(lang, 'enterPubgId')}</b>`, { parse_mode: 'HTML', ...cancelKeyboard(lang) });
  }
  ...
  await ctx.reply(text, { parse_mode: 'HTML', ...Markup.keyboard([[`▶️ ${tr(lang, 'continue')}`], [`❌ ${tr(lang, 'cancel')}`]]).resize() });
  s.mode = 'confirm_order';
}

async function choosePayment(ctx) {
  const lang = langOf(ctx);
  const rows = (data.products().payments || ['СБП', 'Банковские карты РФ', 'USDT']).map(x => [`💳 ${x}`]);
  rows.push([`❌ ${tr(lang, 'cancel')}`]);
  await ctx.reply(`<b>${tr(lang, 'paymentMethod')}</b>`, { parse_mode: 'HTML', ...Markup.keyboard(rows).resize() });
...
async function createDisabledOrder(ctx, paymentMethod) {
  ...
  await ctx.reply(`⚠️ <b>${tr(lang, 'paymentDisabled')}</b>\n\nOrder ID: <code>${order.id}</code>`, { parse_mode: 'HTML', ...mainKeyboard(lang) });
  resetSession(ctx);
}
...
bot.hears([/Главное меню/i, /Main menu/i, /🏠/], async (ctx) => { resetSession(ctx); await sendWelcome(ctx); });
bot.hears([/Язык/i, /Language/i, /🌐/], async (ctx) => {
  const user = ensureUser(ctx);
  user.lang = user.lang === 'ru' ? 'en' : 'ru';
  updateUser(uid(ctx), { lang: user.lang });
  await ctx.reply(tr(user.lang, 'languageChanged'), mainKeyboard(user.lang));
});
bot.hears([/^🛟\s*Помощь$/i, /^Помощь$/i, /^🛟\s*Support$/i, /^Support$/i], (ctx) => ctx.reply(`${tr(langOf(ctx), 'support')}: ${SUPPORT_URL}`, Markup.inlineKeyboard([[Markup.button.url('Open support', SUPPORT_URL)]])));
bot.hears([/^📣\s*Канал$/i, /^Канал$/i, /^📣\s*Channel$/i, /^Channel$/i], (ctx) => ctx.reply(`${tr(langOf(ctx), 'channel')}: ${CHANNEL_URL}`, Markup.inlineKeyboard([[Markup.button.url('Open channel', CHANNEL_URL)]])));
bot.hears([/^ℹ️\s*Информация$/i, /^Информация$/i, /^ℹ️\s*Information$/i, /^Information$/i], async (ctx) => {
  ...
  await ctx.reply(msg, { disable_web_page_preview: true, ...homeButton(lang) });
});
bot.hears([/^💰\s*Купить UC$/i, /^Купить UC$/i, /^💰\s*Buy UC$/i, /^Buy UC$/i], showUcMethod);
bot.hears([/Купить по ID/i, /Buy by ID/i, /🆔/], async (ctx) => { getSession(ctx).mode = 'uc_cart'; await showUcCart(ctx); });
bot.hears([/Купить кодом/i, /Buy code/i, /🔐/], async (ctx) => ctx.reply(langOf(ctx) === 'ru' ? 'Коды будут добавлены позже.' : 'Codes will be added later.', mainKeyboard(langOf(ctx))));
bot.hears([/^⭐\s*Prime Plus$/i, /^Prime Plus$/i], showPrime);
bot.hears([/Отменить/i, /Cancel/i, /❌/], async (ctx) => { resetSession(ctx); await ctx.reply('OK', mainKeyboard(langOf(ctx))); });
bot.hears([/Очистить/i, /Clear/i, /🧹/], async (ctx) => { getSession(ctx).cart = {}; await showUcCart(ctx); });
bot.hears([/Оформить заказ/i, /Checkout/i, /✅/], confirmOrder);
bot.hears([/Продолжить/i, /Continue/i, /▶️/], choosePayment);
bot.hears([/СБП|Банковские карты РФ|USDT/i], async (ctx) => createDisabledOrder(ctx, ctx.message.text.replace('💳 ', '').trim()));
bot.hears([/Топ донатеров/i, /Top donators/i, /🏆/], (ctx) => ctx.reply(tr(langOf(ctx), 'topText'), homeButton(langOf(ctx))));
...
bot.on('text', async (ctx) => {
  ...
  if (s.mode === 'await_pubg_id') {
    if (!isValidPubgId(text)) return ctx.reply(`❗ <b>${tr(lang, 'invalidPubgId')}</b>`, { parse_mode: 'HTML', ...cancelKeyboard(lang) });
    s.pubgId = text;
    return confirmOrder(ctx);
  }
  if (s.mode === 'uc_cart' && /^\d+$/.test(text)) {
  ...
  await ctx.reply(lang === 'ru' ? 'Используйте меню для навигации.' : 'Use the menu to navigate.', mainKeyboard(lang));
});
