...
/*
  IMPORTANT:
  - No bottom Reply Keyboard is used anywhere.
  - All user buttons are Inline Keyboard buttons under bot messages.
  - Inline buttons cannot be forced to exact red/green/blue colors by standard Telegram Bot API.
    The bot uses premium/custom emojis in text and inline layout like the examples.
*/

function mainInlineKeyboard(lang = 'ru') {
  return Markup.inlineKeyboard([
    [Markup.button.webApp(`💝 ${tr(lang, 'openApp')}`, MINI_APP_URL)],
    [Markup.button.callback(`💰 ${tr(lang, 'buyUc')}`, 'menu:uc')],
    [
      Markup.button.callback(`⭐ ${tr(lang, 'prime')}`, 'menu:prime'),
      Markup.button.callback(`ℹ️ ${tr(lang, 'info')}`, 'menu:info')
    ],
    [
      Markup.button.url(`🛟 ${tr(lang, 'support')}`, SUPPORT_URL),
      Markup.button.url(`📣 ${tr(lang, 'channel')}`, CHANNEL_URL)
    ],
    [Markup.button.callback(`🌐 ${tr(lang, 'lang')}`, 'menu:language')]
  ]);
}
function inlineSiteButtons(lang = 'ru') {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback(`🏠 ${tr(lang, 'home')}`, 'menu:home')]
  ]);
}
function homeInline(lang = 'ru') {
  return Markup.inlineKeyboard([[Markup.button.callback(`🏠 ${tr(lang, 'home')}`, 'menu:home')]]);
}
function cancelInline(lang = 'ru') {
  return Markup.inlineKeyboard([
    [Markup.button.callback(`❌ ${tr(lang, 'cancel')}`, 'cancel')],
    [Markup.button.callback(`🏠 ${tr(lang, 'home')}`, 'menu:home')]
  ]);
}
...
async function editOrReply(ctx, text, markup, options = {}) {
  ...
  if (ctx.callbackQuery) {
    try {
      ...
      // If the previous message was a photo/caption, editing text may fail; fall back to a new message.
    }
  }
...
async function showMain(ctx) {
  ...
  await editOrReply(ctx, `🏠 <b>${tr(lang, 'main')}</b>\n\n${CUSTOM.app} <b>${BOT_NAME}</b>`, mainInlineKeyboard(lang));
}
...
async function showUcMethod(ctx) {
  ...
  await editOrReply(ctx, `<b>${tr(lang, 'chooseUcWay')}</b>`, Markup.inlineKeyboard([
    [Markup.button.callback(`🆔 ${tr(lang, 'byId')}`, 'uc:by_id')],
    [Markup.button.callback(`🔐 ${tr(lang, 'byCode')}`, 'uc:by_code')],
    [Markup.button.callback(`❌ ${tr(lang, 'cancel')}`, 'cancel')]
  ]));
}
...
async function showUcCart(ctx) {
  ...
  const rows = products.filter(p => p.active !== false).map(p => {
    ...
    if (q > 0) {
      return [
        Markup.button.callback(`➕ ${p.title} (${q}) - ${rub(p.priceRub)}`, `uc:add:${p.id}`),
        Markup.button.callback(`➖ ${lang === 'ru' ? 'Удалить' : 'Remove'}`, `uc:remove:${p.id}`)
      ];
    }
    return [Markup.button.callback(`➕ ${p.title} - ${rub(p.priceRub)}`, `uc:add:${p.id}`)];
  });
  rows.push([Markup.button.callback(`✍️ ${tr(lang, 'customAmount')}`, 'uc:custom')]);
  rows.push([Markup.button.callback(`✅ ${tr(lang, 'checkout')}`, 'checkout')]);
  rows.push([Markup.button.callback(`🧹 ${tr(lang, 'clearCart')}`, 'cart:clear'), Markup.button.callback(`🏠 ${tr(lang, 'home')}`, 'menu:home')]);
...
async function showPrime(ctx) {
  ...
  const rows = (data.products().prime_plus || [])
    ...
    .map(p => [Markup.button.callback(`⭐ ${p.title} - ${rub(p.priceRub)}`, `prime:select:${p.id}`)]);

  rows.push([Markup.button.callback(`🏠 ${tr(lang, 'home')}`, 'menu:home')]);
...
async function confirmOrder(ctx) {
  ...
  if (hasUc && !s.pubgId) {
    ...
    return ctx.reply(`✅ <b>${tr(lang, 'enterPubgId')}</b>`, { parse_mode: 'HTML', ...cancelInline(lang) });
  }
  ...
  await ctx.reply(text, { parse_mode: 'HTML', ...Markup.inlineKeyboard([
    [Markup.button.callback(`▶️ ${tr(lang, 'continue')}`, 'payment:choose')],
    [Markup.button.callback(`❌ ${tr(lang, 'cancel')}`, 'cancel')]
  ]) });
...
async function choosePayment(ctx) {
  const lang = langOf(ctx);
  const rows = (data.products().payments || ['СБП', 'Банковские карты РФ', 'USDT']).map(x => [Markup.button.callback(`💳 ${x}`, `pay:${x}`)]);
  rows.push([Markup.button.callback(`❌ ${tr(lang, 'cancel')}`, 'cancel')]);
  await editOrReply(ctx, `<b>${tr(lang, 'paymentMethod')}</b>`, Markup.inlineKeyboard(rows));
