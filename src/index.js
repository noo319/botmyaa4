...
function allAdminIds() {
  const ids = new Set([Number(OWNER_ID)]);
  const admins = data.admins().admins || [];
...
function userLabelFromCtx(ctx) {
  const u = ctx.from || {};
  const name = [u.first_name, u.last_name].filter(Boolean).join(' ') || 'Unknown';
  const username = u.username ? `@${u.username}` : 'no_username';
  return `${escapeHtml(name)} | ${escapeHtml(username)} | ID: ${u.id}`;
}
...
function resolveUserId(input) {
  ...
  const users = data.users();
  const found = Object.values(users).find(u => String(u.username || '').toLowerCase() === username);
  return found?.id ? Number(found.id) : null;
}

async function notifyBalanceChanged(userId, amount, newBalance, type = 'add') {
  const amountText = rub(Math.abs(Number(amount || 0)));
  const balanceText = rub(newBalance);
  let text = '';
  if (type === 'add') {
    text = `✅ <b>Баланс пополнен</b>\n\nНа ваш баланс добавлено: <b>${amountText}</b>\nТекущий баланс: <b>${balanceText}</b>`;
  } else if (type === 'set') {
    text = `✅ <b>Баланс обновлён</b>\n\nВаш текущий баланс: <b>${balanceText}</b>`;
  } else if (type === 'remove') {
    text = `➖ <b>Баланс изменён</b>\n\nС баланса списано: <b>${amountText}</b>\nТекущий баланс: <b>${balanceText}</b>`;
  }
  try { await bot.telegram.sendMessage(userId, text, { parse_mode: 'HTML' }); } catch {}
}

function adminPanelText() {
  return `🛠 <b>MD STORE Admin Panel</b>
...
<code>/addcoupon CODE PERCENT</code>
<code>/delcoupon CODE</code>
<code>/coupons</code>
<code>/ban USER_ID</code>
<code>/unban USER_ID</code>
<code>/setmin USER_ID AMOUNT</code>
<code>/resetmin USER_ID</code>
<code>/discount24</code>
<code>/prices</code>
<code>/setprice CAT_KEY PRODUCT_ID PRICE</code>
<code>/discountall PERCENT</code>
<code>/payments</code>
<code>/reply USER_ID MESSAGE</code>
<code>/addproduct id|category|title|base_price|rate|ask_game_id(0/1)</code>
<code>/delproduct PRODUCT_ID</code>
<code>/setrate CATEGORY PERCENT</code>
<code>/setgamerate PERCENT</code>
<code>/setcoderate PERCENT</code>
<code>/backup</code>
<code>/restore</code>
<code>/exportusers</code>
<code>/exportorders</code>
<code>/exportbalances</code>

👮 <b>Admins</b>
<code>/addadmin USER_ID_OR_USERNAME</code>
<code>/deladmin USER_ID_OR_USERNAME</code>
<code>/admins</code>`;
}
...
function mainInlineKeyboard(lang = 'ru') {
  return inlineKeyboard([
    ...
    [cb(lang === 'ru' ? '👤 Профиль' : '👤 Profile', 'menu:profile', 'primary')],
    [
    ...
    [cb(`${tr(lang, 'lang')}`, 'menu:language', 'primary', EMOJI.lang)]
  ]);
}
...
async function showProfile(ctx) {
  ...
  const orders = data.orders().filter(o => Number(o.telegramId) === Number(u.id));
  const spent = orders.reduce((sum, o) => sum + Number(o.totalRub || 0), 0);
  const name = [u.firstName, u.lastName].filter(Boolean).join(' ') || 'MAYKI User';
  const username = u.username ? `@${u.username}` : 'no_username';

  const text = lang === 'ru'
    ? `👤 <b>Ваш профиль</b>\n\n<b>Имя:</b> ${escapeHtml(name)}\n<b>Username:</b> ${escapeHtml(username)}\n<b>Telegram ID:</b> <code>${u.id}</code>\n\n💰 <b>Баланс:</b> ${rub(u.balanceRub || 0)}\n🛒 <b>Заказов:</b> ${orders.length}\n💳 <b>Потрачено:</b> ${rub(spent)}`
    : `👤 <b>Your profile</b>\n\n<b>Name:</b> ${escapeHtml(name)}\n<b>Username:</b> ${escapeHtml(username)}\n<b>Telegram ID:</b> <code>${u.id}</code>\n\n💰 <b>Balance:</b> ${rub(u.balanceRub || 0)}\n🛒 <b>Orders:</b> ${orders.length}\n💳 <b>Spent:</b> ${rub(spent)}`;
...
async function createDisabledOrder(ctx, paymentMethod) {
  ...
  const currentBalance = Number(user.balanceRub || 0);

  if (currentBalance <= 0) {
    const orderId = `MYK-${Date.now()}`;
    const text = `⚠️ <b>${tr(lang, 'paymentDisabled')}</b>\n\nOrder ID: <code>${orderId}</code>`;
    await editOrReply(ctx, text, mainInlineKeyboard(lang));
    await notifyAdmins(`⚠️ <b>Order blocked: no balance</b>\n\nUser: ${userLabelFromCtx(ctx)}\nOrder ID: <code>${orderId}</code>\nAmount: <b>${rub(total)}</b>\nPayment: ${escapeHtml(paymentMethod)}\n\nReply:\n<code>/reply ${userId} MESSAGE</code>`);
    resetSession(ctx);
    return;
  }
  ...
  if (currentBalance < total) {
    const text = lang === 'ru'
      ? `❗ <b>Недостаточно баланса</b>\n\nВаш баланс: <b>${rub(currentBalance)}</b>\nСумма заказа: <b>${rub(total)}</b>\n\nПожалуйста, пополните баланс и попробуйте снова.`
      : `❗ <b>Insufficient balance</b>\n\nYour balance: <b>${rub(currentBalance)}</b>\nOrder amount: <b>${rub(total)}</b>\n\nPlease top up your balance and try again.`;
    await editOrReply(ctx, text, mainInlineKeyboard(lang));
    await notifyAdmins(`⚠️ <b>Order blocked: insufficient balance</b>\n\nUser: ${userLabelFromCtx(ctx)}\nBalance: <b>${rub(currentBalance)}</b>\nOrder amount: <b>${rub(total)}</b>\nPayment: ${escapeHtml(paymentMethod)}\n\nReply:\n<code>/reply ${userId} MESSAGE</code>`);
    resetSession(ctx);
    return;
  }
  ...
  users[userId] = { ...user, balanceRub: currentBalance - total, updatedAt: nowIso() };
  data.saveUsers(users);

  const itemsText = order.items.map(x => `${escapeHtml(x.title)} × ${x.qty}`).join('\n');
  const targetText = order.primeTarget ? `Telegram: ${escapeHtml(order.primeTarget)}` : order.pubgId ? `PUBG ID: ${escapeHtml(order.pubgId)}` : '—';

  await notifyAdmins(`✅ <b>New paid order received</b>\n\nOrder ID: <code>${order.id}</code>\nUser: ${userLabelFromCtx(ctx)}\nService: <b>${escapeHtml(order.orderType || 'order')}</b>\nTarget: <code>${targetText}</code>\nItems:\n${itemsText}\n\nAmount: <b>${rub(total)}</b>\nBalance after: <b>${rub(users[userId].balanceRub)}</b>\nPayment: ${escapeHtml(paymentMethod)}\n\nReply:\n<code>/reply ${userId} MESSAGE</code>`);

  const text = lang === 'ru'
    ? `✅ <b>Заказ принят</b>\n\nOrder ID: <code>${order.id}</code>\nСумма списана с вашего баланса: <b>${rub(total)}</b>\nОстаток баланса: <b>${rub(users[userId].balanceRub)}</b>\n\nАдминистратор получил уведомление о вашем заказе.`
    : `✅ <b>Order accepted</b>\n\nOrder ID: <code>${order.id}</code>\nAmount deducted from your balance: <b>${rub(total)}</b>\nRemaining balance: <b>${rub(users[userId].balanceRub)}</b>\n\nThe administrator has received your order notification.`;
...
// Block banned users globally and notify admins about new users/messages.
bot.on('message', async (ctx, next) => {
  const id = uid(ctx);
  const beforeUsers = data.users();
  const isNewUser = !beforeUsers[id];
  ...
  if (isNewUser) {
    await notifyAdmins(`🆕 <b>New user entered the bot</b>\n\n${userLabelFromCtx(ctx)}\nLanguage: ${escapeHtml(ctx.from?.language_code || '—')}\n\nReply:\n<code>/reply ${id} MESSAGE</code>`);
  }

  if (!isAdminId(id)) {
    const rawText = ctx.message?.text || ctx.message?.caption || `[${Object.keys(ctx.message || {}).filter(k => !['message_id','from','chat','date'].includes(k)).join(', ') || 'message'}]`;
    const safeText = escapeHtml(rawText).slice(0, 3500);
    await notifyAdmins(`📩 <b>New message from user</b>\n\n${userLabelFromCtx(ctx)}\n\nMessage:\n<code>${safeText}</code>\n\nReply:\n<code>/reply ${id} MESSAGE</code>`);
  }
...
bot.action('menu:profile', async (ctx) => { await ctx.answerCbQuery().catch(() => {}); await showProfile(ctx); });
...
bot.command('admin', (ctx) => { if (!adminOnly(ctx)) return; ctx.reply(adminPanelText(), { parse_mode: 'HTML' }); });

bot.command('addbalance', async (ctx) => { if (!adminOnly(ctx)) return; const [rawId, amount] = cmdArgs(ctx); const id = resolveUserId(rawId) || rawId; const addAmount = Number(amount || 0); const u = updateUser(id, { balanceRub: Number((data.users()[id]?.balanceRub || 0)) + addAmount }); await ctx.reply(`✅ Balance: ${id} = ${rub(u.balanceRub)}`); await notifyBalanceChanged(id, addAmount, u.balanceRub, 'add'); });
bot.command('removebalance', async (ctx) => { if (!adminOnly(ctx)) return; const [rawId, amount] = cmdArgs(ctx); const id = resolveUserId(rawId) || rawId; const removeAmount = Number(amount || 0); const u = updateUser(id, { balanceRub: Number((data.users()[id]?.balanceRub || 0)) - removeAmount }); await ctx.reply(`✅ Balance: ${id} = ${rub(u.balanceRub)}`); await notifyBalanceChanged(id, removeAmount, u.balanceRub, 'remove'); });
bot.command('setbalance', async (ctx) => { if (!adminOnly(ctx)) return; const [rawId, amount] = cmdArgs(ctx); const id = resolveUserId(rawId) || rawId; const u = updateUser(id, { balanceRub: Number(amount || 0) }); await ctx.reply(`✅ Balance: ${id} = ${rub(u.balanceRub)}`); await notifyBalanceChanged(id, amount, u.balanceRub, 'set'); });
bot.command('check', (ctx) => { if (!adminOnly(ctx)) return; const [rawId] = cmdArgs(ctx); const id = resolveUserId(rawId) || rawId; const u = data.users()[id]; if (!u) return ctx.reply('Not found'); ctx.reply(JSON.stringify(u, null, 2)); });
...
bot.command('addadmin', (ctx) => { if (uid(ctx) !== OWNER_ID) return ctx.reply('Owner only'); const [rawId] = cmdArgs(ctx); if (!rawId) return ctx.reply(tr(langOf(ctx), 'addAdminUsage')); const n = resolveUserId(rawId); if (!n) return ctx.reply('User not found. Ask the user to start the bot first, then try again.'); const obj = data.admins(); if (!obj.admins.map(Number).includes(Number(n))) obj.admins.push(Number(n)); data.saveAdmins(obj); ctx.reply(`✅ Admin added: ${n}`); });
bot.command('deladmin', (ctx) => { if (uid(ctx) !== OWNER_ID) return ctx.reply('Owner only'); const [rawId] = cmdArgs(ctx); if (!rawId) return ctx.reply(tr(langOf(ctx), 'delAdminUsage')); const n = resolveUserId(rawId) || Number(rawId); const obj = data.admins(); obj.admins = obj.admins.filter(x => Number(x) !== Number(n)); data.saveAdmins(obj); ctx.reply(`✅ Admin removed: ${n}`); });
